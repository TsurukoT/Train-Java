...
class Simulator {
 ...
 public SimpleSet<Person> getMurderSuspects(String murderLocation) {
  //The possible suspects of murder which will be returned.
   SimpleSet<Person> suspectNames = new SimpleSet<>();
   //The people in the previous car of murderlocation that is not empty.
   SimpleSet<Person> pepInCarPrev = new SimpleSet<>();
   //The people in the next car of murderLocation that is not empty.
   SimpleSet<Person> pepInCarNext = new SimpleSet<>();
   //The Car which murder happened.
   Car murLoc = new Car(murderLocation);
   //Object array of people to iterate through.
   Object [] peap = people.valuesToArray();
   //Object array of car to iterate through.
   Object [] car = cars.valuesToArray();
   int exist = 0;
   for (int i = 0; i < car.length; i++){
     if ((((Car)car[i]).getName()).equals(murLoc.getName())){
       exist++;
       murLoc = ((Car) car[i]);
     }
   }
   ...
   for (int i = 0; i < peap.length; i++){
     if ((((Person) peap[i]).getCurrentCar()).equals(murLoc)){
       suspectNames.add((Person) peap[i]);
     }
   }
   //previous car of murLoc for iteration
   Car murLocPrev = murLoc;
   //next car of murLoc for iteration
   Car murLocNext = murLoc;
   while (((murLoc.getPrevious() != null) || (murLoc.getNext() != null)) && (suspectNames.size() == 0)){
     if ((murLoc.getPrevious() != null) && (murLoc.getNext() == null)){
       while(murLoc.getPrevious() != null){
         murLocPrev = murLocPrev.getPrevious();
         pepInCarPrev = checkCar(murLocPrev);
         if (pepInCarPrev.size() != 0){
           break;
         }
       }
       suspectNames = pepInCarPrev;
     }
     else if ((murLoc.getNext() != null) && (murLoc.getPrevious() == null)){
       while(murLoc.getNext() != null){
         murLocNext = murLocNext.getNext();
         pepInCarNext = checkCar(murLocNext);
         if (pepInCarNext.size() != 0){
           break;
         }
       }
       suspectNames = pepInCarNext;
     }
       ...
       suspectNames = pepInCarPrev;
       Object [] pepInCarNextArr = pepInCarNext.valuesToArray();
       for (Object o : pepInCarNextArr){
         suspectNames.add((Person) o);
       }   
     }
   }
   return suspectNames;
 ...
 /**
  *  Helper method of getMurderSuspects() above. It will get the people with in the provided car.
  *  @param guessLoc - the Car that will be checked who are on it.
  *  @return a SimpleSet of person that are on the provided car.
  */
 private SimpleSet<Person> checkCar(Car guessLoc){
   SimpleSet<Person> possibleSuspects = new SimpleSet<>();
   Object [] peap = people.valuesToArray();
   for (Object p : peap){
     if (((Person)p).getCurrentCar().equals(guessLoc)){
       possibleSuspects.add((Person) p);
     }
   }
